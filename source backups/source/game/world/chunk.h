...
namespace Wanderlust
{
	...
	public:
		...
		//virtual void setBlockLayerBase(unsigned short); //Layer ID.
		//virtual void setBlockLayerStretch(unsigned short); //Layer ID.

		virtual char* Save();
		virtual unsigned int SaveSize(); //In bytes.
		virtual void DoneSaving(); //Do we no longer need the buffer returned by Save()?

		virtual unsigned int Load(char*, unsigned int); //Arguments are buffer and size in bytes. Returns size loaded.
