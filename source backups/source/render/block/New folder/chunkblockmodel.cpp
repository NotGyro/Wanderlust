			...
			for(unsigned short SideIter = 0; SideIter < 6; SideIter++) //Build the buffer
			{
				...
				for(unsigned short FaceIter = 0; FaceIter < mNumFacesDrawnPer[SideIter]; FaceIter++)
				{
					for(unsigned short VertIter = 0; VertIter < 6; VertIter++)
					{
						...
						switch (NumToDirection[SideIter])
						{
						case POSITIVE_X: //Fall through
						case NEGATIVE_X: //X axis, use Y and Z for UV values. 
							{
							mUVBuffer[SideIter][(FaceIter * 6 /*Vertexes per face.*/) + (VertIter*2) /*UV values per vert*/] =
								gSides[((SideIter*FLOATS_PER_FACE)+(VertIter*3))+1]; //Y Value
							mUVBuffer[SideIter][(((FaceIter * 6 /*Vertexes per face.*/) * 2) /*UV values per vert*/ + (VertIter*2)) /*UV values per vert*/ + 1] =
								gSides[((SideIter*FLOATS_PER_FACE)+(VertIter*3))+2]; //Z Value
							}
							break;
						case POSITIVE_Y:
						case NEGATIVE_Y://Y axis, use X and Z for UV values. 
							{
							mUVBuffer[SideIter][(FaceIter * 6 /*Vertexes per face.*/) + (VertIter*2) /*UV values per vert*/] =
								gSides[((SideIter*FLOATS_PER_FACE)+(VertIter*3))+0]; //X Value
							mUVBuffer[SideIter][(((FaceIter * 6 /*Vertexes per face.*/) * 2) /*UV values per vert*/ + (VertIter*2)) /*UV values per vert*/ + 1] =
								gSides[((SideIter*FLOATS_PER_FACE)+(VertIter*3))+2]; //Z Value
							}
							break;
						case POSITIVE_Z:
						case NEGATIVE_Z://Z axis, use X and Y for UV values. 
							{
							mUVBuffer[SideIter][(FaceIter * 6 /*Vertexes per face.*/) + (VertIter*2) /*UV values per vert*/] =
								gSides[((SideIter*FLOATS_PER_FACE)+(VertIter*3))+0]; //X Value
							mUVBuffer[SideIter][(((FaceIter * 6 /*Vertexes per face.*/) * 2) /*UV values per vert*/ + (VertIter*2)) /*UV values per vert*/ + 1] =
