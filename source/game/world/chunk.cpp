...
namespace Wanderlust
{
	...
	chunk::chunk()
	{
		...
		//mBlockLayerStretch = static_cast<blocklayer*>(new byteblocklayer); //Testing purposes only.
		addLayer((chunklayer*)mBlockLayerBase);
		//addLayer((chunklayer*)mBlockLayerStretch);
	...
	datachainnode* chunk::getSaver()
	{
		if(!mDoingIO)
		{
			mDoingIO = true;
			datachainnode* First = 0;
			datachainnode* Current = 0;
			datachainnode* LayerChain = 0;
			for(std::vector<chunklayer*>::iterator i = mLayers.begin(); i != mLayers.end(); i++)
			{
				LayerChain = (*i)->getSaver();
				if(Current != 0)
				{
					Current->Next = LayerChain;
				}
				Current = LayerChain;
				if(First == 0)
				{
				...
				if(Current != 0)
				{
					Current->Header.ExtraSize = sizeof(unsigned long);
					Current->Header.HeaderData = new char[sizeof(unsigned long)];
					unsigned long Revision = 0;
					memcpy(Current->Header.HeaderData, (void*)&Revision, (sizeof(unsigned long)));
				}
	...
	void chunk::DoneSaving() //Do we no longer need the data chain returned by GetSaver()?
	{
		mDoingIO = false;
	}
	datachainnode* chunk::getLoader(datachainnode* Structure) //Argument is structure chain. Returns loader
	{
		if(!mDoingIO)
		{
			mDoingIO = true;
			datachainnode* FirstReturn = 0;
			datachainnode* CurrentReturn = 0;
			datachainnode* CurrentPass = 0;
			datachainnode* NextPass = Structure;
			datachainnode* FirstPass = 0;
			for(std::vector<chunklayer*>::iterator i = mLayers.begin(); i != mLayers.end(); i++)
			{
				CurrentPass = NextPass;
				FirstPass = CurrentPass;
				if(CurrentPass->Next != 0)
				{
					if(CurrentPass->Next->Header.Depth != 0)
					{
						while((CurrentPass->Next != 0)) 
						{
							if(CurrentPass->Next->Header.Depth == 0)
							{
								break;
				...
				if(FirstReturn == 0)
				{
					FirstReturn = (*i)->getLoader(FirstPass);
					CurrentReturn = FirstReturn;
				}
				else
				{
					CurrentReturn->Next = (*i)->getLoader(FirstPass);
					CurrentReturn = CurrentReturn->Next;
				}
				//DeleteDataChainStructure(FirstPass);
				if(NextPass == 0)
				{
					break;
				}
	...
	void chunk::DoneLoading() //Do we no longer need the data referenced by the chain in getLoader()?
	{
		mDoingIO = false;
	}
	/*
